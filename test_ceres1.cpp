...
struct BallisticResidual {
    ...
    bool operator()(const T* const params, T* residuals) const {
        T x0 = params[0], y0 = params[1];
        T vx0 = params[2], vy0 = params[3];
        T g = params[4], k = params[5];

        T dt = T(t_);
        T e = ceres::exp(-k * dt);

        residuals[0] = x0 + vx0/k*(T(1.0)-e) - T(x_obs_);
        residuals[1] = y0 + (vy0+g/k)/k*(T(1.0)-e) - g/k*dt - T(y_obs_);
        return true;
    }
...
int main() {
    ...
    Scalar lower_hsv(90,150,150), upper_hsv(110,255,255);
    Mat frame, hsv, mask;
    ...
    while (cap.read(frame) && !frame.empty()) {
        frame_idx++;
        if (frame_idx % 2 != 1) continue; // 每隔2帧
    ...
    if (points_math.size() < 6) {
        fprintf(stderr, "Too few points (<6). Exit.\n");
        return 0;
    }

    double x0_init = points_math.front().x;
    double y0_init = points_math.front().y;
    size_t useN = min((size_t)5, points_math.size()-1);
    ...
    double g_init = 500.0, k_init = 0.1;
